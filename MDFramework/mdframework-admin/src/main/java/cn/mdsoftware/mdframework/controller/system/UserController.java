package cn.mdsoftware.mdframework.controller.system;
...
import cn.mdsoftware.mdframework.service.system.UserService;
...
//		if ()
		...
		sysUserDO.setUserId(userDO.getUserId());
		sysUserDO.setPassword(MD5Utils.encrypt(userDO.getUsername(), password.trim()));
		if (userService.updateInfo(sysUserDO) > 0) {
			return R.ok();
		}
		return R.error();
	}
