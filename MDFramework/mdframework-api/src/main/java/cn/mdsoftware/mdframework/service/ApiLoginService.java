...
public class ApiLoginService {
	...
	public void login(LoginReq loginReq, HttpResponse<LoginResp> resp) {
		...
		data.setHeaderUrl(CollectionUtils.isEmpty(photoList) ? "":photoList.get(0).getPhotoUrl());
		...
		resp.setData(data);
		redisTemplate.opsForValue().set(Constants.RedisKey.APP_SESSION.concat(entity.getPhone()), token, 1, TimeUnit.DAYS);
	}
